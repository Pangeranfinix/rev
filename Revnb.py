...
def logo():
  ...
  [#] Create By :: OWNER
          ...
          < ReverseIPLookup >   < Telegram : @vxynb  >
	     Banyuwangi Xploiter - Bandung Xploiter
"""
	...
	try:
		...
		if 'null' in grab.text:
			...
			for domain in result: 
				ASU = domain.replace('cpanel.','').replace('cpcalendars.','').replace('cpcontacts.','').replace('webmail.','').replace('webdisk.','').replace('hostmaster.','').replace('mail.','').replace('ns1.','').replace('ns2.','').replace('api.','').replace('ns3.','').replace('root.','').replace('autogrill.dev.','').replace('development.','').replace('.cdn.cloudflare.net.','')
				open('Rev.txt', 'a').write('http://' + ASU + "\n")
	except:
			pass
